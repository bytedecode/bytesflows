...
1. Does the workflow need rotating or sticky sessions?
1. Does the runtime require HTTP, HTTPS, or SOCKS5?
1. What failure rate is acceptable before retries become too expensive?
1. How many MB does one useful browser or scraping output consume?
1. Which targets need real validation before scale?
1. How will the team distinguish bad route, wrong market, soft block, parser error, and target-policy response?
...
1. Credentials are accepted without 407 errors.
1. Exit IP, country, city, and ASN match expectations.
1. Real target returns the expected page class.
1. Wrong-market output is counted as failure.
1. Parser failures are separated from route failures.
1. Retry rules change one meaningful variable at a time.
1. Traffic per useful output is measured on a small sample.
1. Results are grouped by workflow, target, market, and session mode.
