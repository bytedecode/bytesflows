...
1. Market and currency match the job requirement.
1. Page class is a normal product or category page.
1. Stock state is explicitly parsed, not inferred from fetch failure.
1. Seller or marketplace source is captured when relevant.
1. Parser version is known.
1. Retry count is within budget.
1. Output is marked usable.
1. Timestamp and cadence match the report.
...
1. Define markets, currencies, and delivery-region assumptions.
1. Decide which pages need browser rendering.
1. Choose rotating or sticky sessions per workflow.
1. Separate collection facts from business facts.
1. Treat wrong-market records as unusable.
1. Treat parser errors separately from stock states.
1. Store promotion context separately from base price.
1. Manually review the first 100 usable records.
1. Estimate traffic per usable price record.
