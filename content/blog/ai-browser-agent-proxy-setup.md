...
2. Which route and session should the browser keep during that task?
3. What evidence proves the result?
4. How much traffic does one useful agent output consume?
...
2. Assign one route and session policy per task.
3. Match browser locale and timezone to the requested market.
4. Require final URL, evidence, and output schema.
5. Classify route failures separately from agent action failures.
6. Stop on proxy authentication errors instead of retrying blindly.
7. Treat wrong-market output as unusable.
8. Review the first 50 tasks manually.
9. Measure traffic per useful task.
10. Keep policy and acceptable-use review outside the agent loop.
