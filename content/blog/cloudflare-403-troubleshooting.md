...
2. The same URL fails with the same response class after one controlled retry.
3. The response is login-gated and the workflow is not authorized.
4. The response indicates wrong market or unsupported region.
5. The crawler stores access pages as business data.
6. Retry traffic exceeds the value of useful outputs.
7. Policy or acceptable-use review is incomplete.
8. Human review cannot explain the failure class.
